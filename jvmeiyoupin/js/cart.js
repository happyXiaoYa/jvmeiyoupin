class Cart {
    ...
    oneGoodsCheckBox() {
        ...
        this.$('.good-checkbox').forEach(input => {
            ...
            input.onclick = function () {
                ...
                //判断当前商品的 input点击的是取消，则此时取消全选
                if (!this.checked) {
                    self.$('.cart-th input').checked = false;
                }
                //点击选中时则判断页面中是否有其他的未选中，如果都选中则全选选中
                if (this.checked) {
    ...
    delGoodsData(tar) {
        ...
        //找到ul上的商品id
        let ul = tar.parentNode.parentNode.parentNode;
        // let id = tar.parentNode.parentNode.parentNode.dataset.id;
        let id = ul.dataset.id;
        // console.log(id);
    ...
    async getCartGoods() {
        ...
        if (status == 200 && data.code == 1) {
            ...
            //单个商品的追加是异步实现的，所以单选按钮绑定点击事件要放在这里
            this.oneGoodsCheckBox();
        }


        //登录过期的处理
        if (status == 200 && data.code == 401) {//如果等率过期则重新登录
            //清除local中存的token和userid
            localStorage.removeItem('token');
            localStorage.removeItem('user_id');
            //跳转登录页面
            location.assign('../html/login.html?ReturnUrl=../html/cart.html');
